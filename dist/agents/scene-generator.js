...
class SceneGeneratorAgent {
    ...
    videoModel;
    storageManager;
    constructor(llm, videoModel, storageManager) {
        this.llm = llm;
        this.videoModel = videoModel;
        this.storageManager = storageManager;
    }
    async generateScene(scene, enhancedPrompt, previousFrameUrl) {
        try {
            console.log(`\n🎬 Generating Scene ${scene.id}: ${scene.timeStart} - ${scene.timeEnd}`);
            console.log(`   Duration: ${scene.duration}s | Shot: ${scene.shotType}`);
            const videoUrl = await this.generateVideo(enhancedPrompt, scene.duration, scene.id, previousFrameUrl);
            let lastFrameUrl;
            try {
    ...
    async sanitizePrompt(originalPrompt) {
    ...
    async generateVideo(prompt, duration, sceneId, startFrame) {
    ...
    async executeVideoGeneration(prompt, duration, sceneId, startFrame) {
        ...
        const videoGenParams = (0, llm_params_1.buildVideoGenerationParams)({
            prompt,
            image: imageParam,
            config: {
                durationSeconds,
    ...
    async extractLastFrame(videoUrl, sceneId) {
        const fs = require('fs');
    ...
    async stitchScenes(videoPaths) {
        ...
        try {
            // 1. Download all videos (Parallelized)
            ...
            // 4. Upload result (Streaming)
            // Using uploadFile streams from disk instead of buffering entire file in memory
